...
from database_service import execute_read_df
...
def read_data_from_db(symbol: str, start: str, end: str) -> pd.DataFrame:
    ...
    query = f"SELECT * FROM stock_price WHERE symbol = '{symbol} and {DATE} BETWEEN '{start} AND '{end}''"
    db = execute_read_df(query)
    return db
...
def write_data(symbol: str,
    ...
    query = f"INSERT INTO strategy_return " \
            f"(symbol, start_date, end_date, annual_return, asset, cost) " \
            f"VALUES ('{symbol}', '{start}', '{end}', {annual}, {asset}, {cost})"
    execute_write(query)
    return None
